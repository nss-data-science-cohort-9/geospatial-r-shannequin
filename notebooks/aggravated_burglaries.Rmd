...
```{r}
# Convert burglaries to a sf object
...
```{r}
# Create plot
burglaries_by_kpop_point_plot <- census_burglaries |> 
    distinct(TRACTCE, .keep_all = TRUE) |> # Keep unique census tracts
    ggplot(aes(x = INC_PER_KPOP, y = KPOP)) + # Set x and y
    geom_point(alpha = 0.5) + # Use point plot
    labs(title = "Burglaries by Population", # Set title label
         subtitle = "Population in Thousands", # Set subtitle label
         x = "Burglaries", # Set x-axis label
         y = "Population / K") # Set y-axis label

burglaries_by_kpop_point_plot
```

```{r}
# Save the plot
ggsave(
    "../plots/burglaries_by_kpop_point_plot.png", # Set path
    plot = burglaries_by_kpop_point_plot, # Select plot
    width = 5, # Set width
    height = 4, # Set height
    dpi = 300 # Set resolution
)
...
```{r}
# Convert negative income to zero
census_burglaries <- census_burglaries |> 
    mutate(median_income = case_when(median_income < 0 ~ 0, .default = median_income))
...
```{r}
# Create plot
burglaries_by_med_income_point_plot <- census_burglaries |> 
    distinct(TRACTCE, .keep_all = TRUE) |>  # Keep unique census tracts
    ggplot(aes(x = INCIDENTS_PER_TRACT, y = median_income, color = KPOP)) +  # Set x and y
    geom_point() + # Use point plot
    labs(title = "Burglaries by Median Income", # Set title label
         subtitle = "Population in Thousands", # Set subtitle label
         x = "Burglaries", # Set x-axis label
         y = "Median Income", # Set y-axis label
         color = "Population / K") # Set fill label

burglaries_by_med_income_point_plot
```
...
# Save the plot
ggsave(
    "../plots/burglaries_by_med_income_point_plot.png", # Set path
    plot = burglaries_by_med_income_point_plot, # Select plot
    width = 5, # Set width
    height = 4, # Set height
    dpi = 300 # Set resolution
)
